...
def setup_csv(out_dir,veh_id):
    ...
    file_name = f'{current_time}_{veh_id}.csv'
    file_path = os.path.join(out_dir,file_name)
    with open(file_path, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['episode', 'hops', 'reward', 'route'])
    return file_path
...
def parseConfig(args):
    ...
    with open(file=yaml_file) as file:
        try:
            ...
            p_config['log_level'] = config['common']['log_level']
            p_config['cfg_file'] = config['common']['cfg_file']
            p_config['net_file'] = config['common']['net_file']
            p_config['det_file'] = config['common']['det_file']
